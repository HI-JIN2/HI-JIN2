# About Me

책임감과 실행력을 바탕으로 프로덕트를 지속적으로 개선하는 **SW Engineer 유진**입니다.

안드로이드 개발을 강점으로 시작해 백엔드와 클라우드까지 확장하며,  
기능 구현을 넘어 **사용자 경험과 운영 안정성**을 함께 고민해왔습니다.

사용자 피드백과 실제 운영 이슈를 바탕으로 문제를 정의하고,  
적절한 기술과 도구를 선택해 **지속 가능한 해결 방식**으로 연결하는 것을 중요하게 생각합니다.

- Strong in **Kotlin, Android**
- Interested in **Backend, Cloud Infrastructure**

<a href="https://awesome-github-stats.azurewebsites.net/index.html??cardType=level&theme=github&preferLogin=false">    
<img alt="HI-JIN2's GitHub Stats" src="https://awesome-github-stats.azurewebsites.net/user-stats/HI-JIN2?cardType=level&theme=github&preferLogin=false" /> 
</a>

---

## Experience

### [SK m&service](https://www.skmnservice.com/pc_new/html/main.skmns)
_**2025.07 ~ 2025.09 | Android Engineer Intern, Mobility Business**_

- Built an Android **Mock GPS tool** for TMAP AUTO debugging and test automation
- Improved large driving log parsing performance by **50.6%** through buffer tuning, batch processing, and regex simplification
- Designed a **Room-based local caching** structure for stable offline scenario replay
- Resolved rendering issues in Compose-WebView integration
- Implemented BLE communication for HUD devices using **Command Queue** and byte-level protocol handling

**Tech**  `Kotlin` `Compose` `Multi-Module` `Build Logic` `Room` `BLE` `Hilt` `JUnit`

### [Ratio](https://trimm.bike/ko)
_**2024.09 ~ 2024.12 | Android Engineer Intern**_

- Developed a **virtual e-bike** app to simulate BLE communication without physical hardware in demo environments
- Replaced legacy `Thread.join()` flow with **Coroutines**, reducing UI blocking from **250ms to 0.5ms**
- Ensured stable background BLE communication using **Foreground Service**
- Implemented user-facing features such as **goal setting** and **achievement rate visualization**
- Improved home screen responsiveness with **Datastore caching** and background refresh

**Tech**  `Kotlin` `XML` `Compose` `BLE` `Service` `Coroutine`

### [Sakak](https://www.sakak.co.kr/)
_**2023.06 ~ 2023.08 | Backend Engineer Intern**_

- Developed a payment detail parsing API for G-Market settlement data
- Processed unstructured HTML/JSON data into normalized fields and stored them in **MongoDB**
- Verified parsing edge cases with **Pytest-based TDD**
- Experienced collaborative backend development through pair programming and code reviews

**Tech**  `Python` `Selenium` `BeautifulSoup` `Flask` `MongoDB`

---

## Main Projects

### [EAT-SSU](https://github.com/EAT-SSU/Android) | [Google Play](https://play.google.com/store/apps/details?id=com.eatssu.android)
_**2023.03 ~ Present | Android Developer / Former Team Leader, PM, AWS Infra Operation**_

A campus meal review service used by **5,000+ users** at Soongsil University.

- Improved **WAU/MAU from 21.8% to 62.4%**
- Built a stable operation workflow with **Crashlytics + Slack**, resolving **15+ major crashes**
- Automated release workflow with **GitHub Actions + Fastlane**
- Solved JWT refresh **race condition** with `Mutex`, reducing duplicate refresh attempts to one
- Re-architected legacy code into **MVVM + Clean Architecture**, with gradual **Multi-Module / Compose migration**
- Introduced **Firebase Remote Config** and background updates with **WorkManager** to improve operational flexibility
- Contributed to backend and infra improvements, including query optimization and AWS operation

**Tech**  `Kotlin` `XML` `Compose` `Clean Architecture` `MVVM` `Hilt` `Retrofit` `Coroutine` `Flow` `WorkManager` `Firebase` `Fastlane` `AWS`


### [ISBN Scanner Mini Library](https://github.com/HI-JIN2/isbn-scanner-mini-library)
_**2024.07 | Personal Project**_

A book registration automation tool built during an overseas volunteer project in Indonesia.

- Automated the flow from **ISBN scan → book lookup → Google Sheets save**
- Reduced registration time per book from **1 minute to 10 seconds**
- Designed the system for immediate use by non-developers with **Google Sheets as a lightweight DB**
- Built and applied the prototype in a constrained field environment

**Tech**  `Kotlin` `ZXing` `Google Books API` `Google Sheets API` `Apps Script`

---

## Awards

- **K-PaaS Digital Social Innovation Contest** — Gold Prize (2nd)
- **AI Online Development Competition Syncathon Season 3** — Grand Prize (2nd)
- **Soongsil University Software Contest** — Gold Prize
- **Soongsil Capstone Design Competition** — Encouragement Prize

---

## Activity

### Kotlin Ecosystem Mentorship (Kotlin Foundation)
- Contributed to the Compose Multiplatform open-source project Heron - PR [#1221](https://github.com/tunjid/heron/pull/1221) [#1242](https://github.com/tunjid/heron/pull/1242)

### GDSC Soongsil
- Speaker: [학교에서 서비스 운영하기](https://www.youtube.com/watch?v=Qx719VFI6No)
- Speaker: [해외봉사가서 개발하기](https://www.youtube.com/watch?v=BcnWM_nqKow)

### UMC
- Vice Lead of a 50-member university IT club
- Co-hosted the first SSU DEVCON with 6 campus IT clubs and **400 participants**

---

## Education & Certification

- **B.S. in Computer Science**, Soongsil University (2021.03 ~ 2025.08)
- **정보처리기사** (2024.12)
- **SQLD** (2025.12)

---

## Links

- GitHub: https://github.com/HI-JIN2
- Velog: https://velog.io/@jini_1514
- Email: qldls0307@naver.com

